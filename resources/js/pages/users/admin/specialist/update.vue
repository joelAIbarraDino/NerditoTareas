...
import { AppPageProps, BreadcrumbItem, Specialist, SpecialistArea } from '@/types';
...
interface specialistEditPageProps extends AppPageProps{
    specialist: Specialist;
    specialistAreas:SpecialistArea[];
}

const page = usePage<specialistEditPageProps>();
const specialist = computed(()=> page.props.specialist);
const specialistAreaArray = computed(()=>page.props.specialistAreas);


const form = useForm({
    name:specialist.value.user.name,
    area:specialist.value.specialist_area.id,
    email:specialist.value.user.email,
    whatsapp:specialist.value.user.whatsapp,
    password:'',
    password_confirmation:'',
});

function submit(){
  form.put(`/specialists/${specialist.value.id}`,{
    preserveScroll:true,
...
<template>
    <Head title="Actualizar especialista"/>
    <AppLayout :breadcrumbs="breadcrumbs" class="relative">
      <LoadingOverlay :show="form.processing" />
      <RecordForm>
        <RecordFormHeader title-form="Actualizar especialista" return-url="/specialists"/>
